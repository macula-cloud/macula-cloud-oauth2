...
public class JustAuthAuthenticationProcessingFilter extends AbstractAuthenticationProcessingFilter {
	...
	@Autowired
	private SubjectPrincipalRepository principalRepository;
	...
	public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response)
		...
		if (authResponse != null) {
			...
			if (user != null) {
				SubjectCredential credential = SubjectCredential.builder().username(user.getUsername()).source(user.getSource().name()).build();
				SubjectPrincipal principal = principalRepository.load(credential);
				if (principal != null) {
					return SecurityUtils.cast(principal);
				} else {
					boolean created = oauth2UserService.createAuthUser(user);
					if (created) {
						principal = principalRepository.load(credential);
						return SecurityUtils.cast(principal);
					}
				}
			}
